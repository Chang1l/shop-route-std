...
//         return(
//             <div>
//                 홍길동님의 나이는 {this.state.age}세 입니다.<br />
...
/*
useEffect()코드는 html랜더링 후에 동작됨.
따라서 오래걸리는 연산작업이나 서버에서 데이터를 가져오는 작업, 타이머를 장착하는 작업등을 기술할 때 사용한다.
html의 rendering 기능이 아닌 다른 부가 작업에 대한 기능을 수행하는 함수라하여 Side Effect에서 따온 명칭임.
*/
function ComLife(){
    ...
    useEffect(()=>{
        ...
        // return이 붙으면 생성,갱신,소멸 시 모두 적용 됨
        return()=>{
            alert('생성, 갱신, 소멸');
            clearTimeout(timer)
        }
...
2. mount(생성)시 1회만 코드를 실행하고 싶은 경우
useEffect(() => {},[]);
...
*/
    ...
    return(
        <div>
            ...
            {
                at === true
                ? <button onClick={()=>ageChk(++age)}>나이변경</button>
                : null
            }
        </div>
    )
